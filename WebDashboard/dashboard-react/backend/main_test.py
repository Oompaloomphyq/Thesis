...
======================================================================
Reads raw .bin files from Dataset_Test/test_main/, runs the full pipeline:
  preprocess → RANSAC → PointPillars → slot match → dashboard WebSocket

Usage:
  python main_test.py

Drop raw .bin files into:
  /home/phyq/Thesis/Dataset_Test/test_main/
"""
...
# CONFIGURATION
# =============================================================================
TEST_DIR  = "/home/phyq/Thesis/Dataset_Test/march16/live_raw_frames4"
LOOP_FRAMES = True        # if True, loop back to first frame after last
...
# =============================================================================
BUFFER_SIZE         = 4   # how many recent frames to remember per slot
OCCUPIED_THRESHOLD  = 3   # how many of those frames must say "occupied"

# One buffer per slot — initialized to all-vacant
slot_history = {
    f"slot_{i}": deque([0] * BUFFER_SIZE, maxlen=BUFFER_SIZE)
    for i in range(1, 17)
}

def smooth_slot_status(raw_status: dict) -> dict:
    """
    Takes the raw per-frame slot_status and returns a smoothed version.
    A slot is only marked occupied if at least OCCUPIED_THRESHOLD of the
    last BUFFER_SIZE frames detected it as occupied.
    """
    smoothed = {}
...
def load_raw(file_path):
    """Load a raw .bin file (x, y, z, r in Livox space)."""
...
def run_inference(model, dataset, points):
    ...
    for i, det in enumerate(detections):
        print(f"    [{i}] x={det[0]:.2f} y={det[1]:.2f} z={det[2]:.2f} "
              f"dx={det[3]:.2f} dy={det[4]:.2f} dz={det[5]:.2f} score={det[7]:.3f}")
    return detections
...
async def main():
    ...
    print(f"  Smoothing  : {OCCUPIED_THRESHOLD}/{BUFFER_SIZE} frames to confirm")
    print("=" * 60 + "\n")
    ...
    try:
        while True:
            ...
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] "
                  f"Frame {frame_idx + 1}/{len(frame_files)} — {file_path.name}")
            ...
            # Step 5b — Temporal smoothing (fixes blinking)
            ...
            # Step 6 — Send smoothed status to dashboard
